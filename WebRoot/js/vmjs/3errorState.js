...
app.controller('errorStateController', [ '$scope', 'services', '$location',
		function($scope, services, $location) {
			...
			function getLineData(errorAnalyseLine,title,subtitle){
				 var chart = {
					      type: 'bar'
					   };
				 var title = {
				 ...
				 for(var i=0;i<errorAnalyseLine.length;i++){
					 ...
					 categories.push(obj.Detecter_Equipment_Name);
			...
			errorState.findDrawData = function(){
				...
				getPieDraw1(errorState.limit.startTime,errorState.limit.endTime);
				getLineDraw1(errorState.limit.startTime,errorState.limit.endTime);
				$("#drawAnalyse").empty();
				$("#drawAnalyse").append("分析结果：报警次数最多的设备为展会演示中的现场供配电检测，其最可能的原因是A相负载高。");
			}
			...
			function getPieDraw(){
				var colors = Highcharts.getOptions().colors;
				   ...
				   var data = [{
			            ...
			            drilldown: {
			                name: '展会演示报警次数',
			                categories: ['现场供配电检测', '1#变压器', '2#变压器', '现场给排水', '3#水泵房' , '4#水泵房'],
			                data: [9.85, 5.35, 3.00, 25.06, 2.81, 8.00],
			                color: colors[0]
			            }
			        }, {
			            y: 21.63,
			            ...
			            drilldown: {
			                name: '清远凤城郦都报警次数',
			                categories: ['1#变压器', '2#变压器', '2#配电房', '3#配电房' , '3#水泵房' ,'4#水泵房'],
			                data: [0.20, 7.00, 0.83, 1.58, 6.12, 5.43],
			                color: colors[3]
			            }
			        }, {
			            y: 23.26,
			            ...
			            drilldown: {
			                name: '光明迈瑞报警次数',
			                categories: ['1#变压器', '2#变压器', '2#配电房', '3#配电房' , '4#配电房' , '2#水泵房' ,'3#水泵房' , '4#水泵房'],
			                data: [10.51, 2.31, 0.56, 3.45, 4.37, 1.02, 0.64, 0.40],
			                color: colors[2]
			            }
			        }];
				   var browserData = [];
			...
			function getPieDraw1(startTime,endTime){
				var colors = Highcharts.getOptions().colors;
				   ...
				   var title = {
				      text: startTime+"至"+endTime+'各小区报警分析'
				   };      
				   var yAxis = {
				      title: {
				         text: 'Total percent market share'
			...
			function getLineDraw(){
				var chart = {
					   ...
					   var xAxis = {
					      categories: ['展会演示-现场给排水','光明迈瑞-1#变压器','展会演示-现场供配电检测','展会演示-4#水泵房','清远凤城郦都-2#变压器',
					                   '清远凤城郦都-3#水泵房','清远凤城郦都-4#水泵房','展会演示-1#变压器','光明迈瑞-4#配电房','光明迈瑞3#配电房'],
					      title: {
					         text: null
					      }
					   };
					   ...
					   var series= [{
					         name: '报警次数',
					            data: [183,77,72,59,51,45,40,39,31,25]
					        }
					   ];     
