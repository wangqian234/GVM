...
app.factory('services', [ '$http', 'baseUrl', function($http, baseUrl) {
	...
	services.getOperaDetails = function(data){
		return $http({
			method : 'post',
			url : baseUrl + 'operaState/getOperaDetails.do',
			data : data
		});
	};
...
app.controller('operaStateController', [ '$scope', 'services', '$location',
		function($scope, services, $location) {
			...
			operaState.getOperaDetails = function(SensorId,olList){
				services.getOperaDetails({
					sensorId : SensorId
				}).success(function(data){
					$(".modal-content").show();
					var arrTime = new Array();
					var arrValue = new Array(); 
					operaState.operaDetList = data.list;
					for(var i=0;i<operaState.operaDetList.length;i++){
						arrTime.push(operaState.operaDetList[i].Detector_SensorData_Time);
						arrValue.push(parseInt(operaState.operaDetList[i].Detector_SensorData_Value));
					}
					 var data = {
							 elementId:'#containerLine',
							 xAxisUnit:arrTime,
							 yAxisUnit:olList.Detector_Sensor_Name+olList.Detector_Sensor_Unit,
							 title:'',
							 name:olList.Detector_Sensor_Name,
							 value:arrValue
					        };
					 console.log(data);
			...
			operaState.getWhereDraw1 = function(){
				$(".modal-content").show();
				var dataS1 = {
			    		 value: [[161.2, 51.6], [167.5, 59.0], [159.5, 49.2], [157.0, 63.0], [155.8, 53.6],
			     ...
			     var scatter1 = new Scatter(dataS1);
			     scatter1.init();
			}
			operaState.getWhereDraw2 = function(){
				$(".modal-content").show();
				var dataS1 = {
			    		 value: [[174.0, 65.6], [175.3, 71.8], [193.5, 80.7], [186.5, 72.6], [187.2, 78.8],
			...
			operaState.myFunction = function(){
				$('.col-sm-3').tooltip();
				operaState.selectEquipList(sessionStorage.getItem("state"));
			}
			operaState.selectEquipList = function(state){
				sessionStorage.setItem("state",state);
				if(sessionStorage.getItem("state") =='1' && sessionStorage.getItem("project") == '1'){
					$("#getWhere").show();
				}else{
					$("#getWhere").hide();
				}
			...
			function initPage() {
				...
				if ($location.path().indexOf('/testIndex') == 0) {
					...
					var dataS1 = {
				     ...
				     var scatter2 = new Scatter(dataS2);
				     scatter2.init();
			...
			initPage();
			var aa = window.setInterval(operaState.selectEquipList(sessionStorage.getItem("state")), 1000);
		} ]);
